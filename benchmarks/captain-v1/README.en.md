...
| [L1 question book](candidate/L1-题本.md) | Candidate | 20 boundary questions + 20 case-file questions |
...
| [Scoring anchors](examiner/cases.json) | Examiner, must be isolated | 61 units, 157 checkpoints |
...
The examiner still has to arrange candidate model / harness onboarding, simulated message logging, event release and rebuilding the candidate's context. The CLI is currently called by the examiner on the candidate's behalf; the selected real candidate LLM is called, but no real crew or IM is started. Task books have their pickup and fixture return simulated by book.execute, and semantic quality is still reviewed by the examiner; simulated pickup is not called a real crew first-pass completion rate. A program regression PASS is not a candidate model PASS.
...
Use run_l1_exam.py for headless L1 with an empty native tool list. The unchanged 1.1.0 version label covers both the historical 142-check and new 157-check banks: record the source commit/hash. Reported reasoning depth and configured effort are separate metadata; do not infer unknown settings.
